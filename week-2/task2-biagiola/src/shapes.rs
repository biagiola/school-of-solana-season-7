...
impl Rectangle {
    ...
    pub fn new(width: f64, height: f64) -> Result<Self, Error> {
        match width >= 0.0 {
            true => {
                match height >= 0.0 {
                    true => Ok(Self { width, height }),
                    false => Err(Error::InvalidHeight),
                }
            },
            false => Err(Error::InvalidWidth),
        }
    }
...
// 
// Hint: Use std::f64::consts::std::f64::consts::PI to calculate the area and circumference of the circle.
